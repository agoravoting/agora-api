...
CREATE FUNCTION set_vote(v TEXT, vh TEXT, eid TEXT, vid TEXT, theip TEXT, max_writes INT)
RETURNS BOOL AS $$ BEGIN BEGIN INSERT INTO votes(vote, vote_hash, election_id, voter_id, ip) VALUES (v, vh, eid, vid, theip); RETURN FOUND; EXCEPTION WHEN unique_violation THEN UPDATE votes SET vote = v, vote_hash = vh, ip = theip, modified = current_timestamp, write_count = write_count + 1 WHERE voter_id = vid and election_id = eid and write_count < max_writes; RETURN FOUND; END; END; $$
-- RETURNS VOID AS $$ BEGIN BEGIN INSERT INTO votes(vote, vote_hash, election_id, voter_id, ip) VALUES (v, vh, eid, vid, theip); RETURN; EXCEPTION WHEN unique_violation THEN UPDATE votes SET vote = v, vote_hash = vh, ip = theip, modified = current_timestamp, write_count = write_count + 1 WHERE voter_id = vid and election_id = eid and write_count < 1; IF NOT found THEN RAISE EXCEPTION 'Could not update on election_id --> %, voter_id ---> %', eid, vid; END IF; RETURN; END; END; $$
LANGUAGE plpgsql;
-- +goose Down
-- SQL section 'Down' is executed when this migration is rolled back

DROP TABLE votes;
DROP FUNCTION set_vote(vote TEXT, vote_hash TEXT, election_id TEXT, voter_id TEXT, theip TEXT, max_writes int);
